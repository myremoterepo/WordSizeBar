package com.browser2345.wordsizebar.wordsizebar;

import com.browser2345.wordsizebar.R;
...
/**
 * 参考https://github.com/edmodo/range-bar
 * RangeBar是一个类似SeekBar的控件，有带刻度的bar和thumb两种元素。thumb可以拖拽到bar的任意位置，但只允许停止到最近的刻度线上。
 * RangeBar的使用者可以通过{@link RangeBar}里的OnRangeBarChangeListener来监听thumb的移动。
 */
public class RangeBar extends View {
     ...
     *
     * @param thumbImageNormalID Integer specifying the resource ID of the image to be drawn as the normal
     *                           thumb.
     */
     ...
     *
     * @param thumbImagePressedID Integer specifying the resource ID of the image to be drawn as the pressed
     *                            thumb.
     */
    ...
    /**
     * Sets the location of thumb according to the developer's choice.
     * Numbered from 0 to mTickCount - 1 from the left.
     *
     * @param thumbIndex Integer specifying the index of the left thumb
     */
    public void setThumbIndices(int thumbIndex) {
        if (indexOutOfRange(thumbIndex)) {
            throw new IllegalArgumentException("A thumb index is out of bounds. Check that it is between 0 " +
                    "and mTickCount - 1");
        } else {
            if (mFirstSetTickCount)
            ...
            if (mListener != null) {
                mListener.onIndexChangeListener(this, thumbIndex);
            }
        }
    ...
    private void rangeBarInit(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            mThumbColorNormal = ta.getColor(R.styleable.RangeBar_thumbColorNormal,
    ...
    public interface OnRangeBarChangeListener {
        ...
        void onIndexChangeListener(RangeBar rangeBar, int thumbIndex);
    }
}
